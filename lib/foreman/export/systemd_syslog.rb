...
class Foreman::Export::SystemdSyslog < Foreman::Export::Base
  ...
  def export
    ...
    engine.each_process do |name, process|
      ...
      write_template "systemd-syslog/process.service.erb", service_fn, binding
      ...
      write_template "systemd-syslog/process_master.target.erb", "#{app}-#{name}.target", binding
      process_master_names << "#{app}-#{name}.target"
    end

    write_template "systemd-syslog/master.target.erb", "#{app}.target", binding
  end
end